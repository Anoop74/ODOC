...
$np=$_POST['np'];
$cp=$_POST['cp'];
$q="select password from login where rid='".$_SESSION['uid']."' and password='$p' and type='admin'";
...
if($r=mysqli_fetch_array($e))
	{
	    if($np==$cp)
		 {
		 $up="UPDATE `login` SET `password`= '$np' WHERE rid='".$_SESSION['uid']."' and type='admin'";
		    mysqli_query($con,$up);
			$upd="UPDATE `hospital` SET `password`= '$np' WHERE id='".$_SESSION['uid']."'";
...
<form method="post">
<table>
<tr><td>CURRENT PASSWORD :</td><td><input type="password" name="p" id="p" class="form-control"/></td></tr>
<tr><td>NEW PASSWORD :</td><td><input type="password" name="np" id="np" class="form-control"/></td></tr>
<tr><td>CONFIRM PASSWORD :</td><td><input type="password" name="cp" id="cp" class="form-control"/></td></tr>
<tr><td><td><input type="submit" name="SAVE" value="SAVE" onClick="return validation()" class="btn submit_btn"/></td></td></tr>
</table>
</form>
