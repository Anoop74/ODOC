...
function date_check()
{
 var dtt=document.getElementById('d').value;
...
<form method="post">
<center><table>
<tr><td>NAME :</td><td><input type="text"  name="nm" class="form-control"  value="<?php echo $r['name'];?>" required/></td></tr>
<tr><td>AGE :</td><td><input type="text"  name="ag" class="form-control" value="<?php echo $r['age'];?>" required/></td></tr>
<tr><td>GENDER :</td><td>MALE<input type="radio"   name="g" value="Male" <?php if($r[4]=="Male") echo "checked";?>/>FEMALE<input type="radio"  name="g" value="Female" <?php if($r[4]=="Female") echo "checked";?>/></td></tr>
<tr><td>PLACE :</td><td><input type="text" class="form-control" name="pl" required/></td></tr>
<tr><td>BOOKING DATE :</td><td><input type="date" name="d" id="d" class="form-control" required onChange="return date_check();"/></td></tr>

</table><input type="submit" name="book" value="BOOK" class="btn submit_btn"/></center>
...
if(isset($_POST['book']))
{
	...
	$q="select max(token_no) as tno from token where did='$idd' and bdate='".$_POST['d']."'";
	$x=mysqli_query($con,$q);
	$f=mysqli_fetch_array($x);
	$t=$f['tno'];
	$tk=$t+1;
	
	$a="SELECT * FROM `doctor_details` WHERE did='$idd'";
	$ax=mysqli_query($con,$a);
	$m=mysqli_fetch_array($ax);
	$mt=$m['max_token'];
	if($tk>$mt)
	{
		echo "Booking Completed..please select another day";
	}
	else
	{
	
		$sql="INSERT INTO `token`(`tid`, `did`, `pid`, `bdate`, `date`, `token_no`, `name`, `age`, `gender`, `place`, `status`) VALUES ('','$idd','".$_SESSION['uid']."','$d','$dt','$tk','$nm','$ag','$g','$pl','booked')";
		mysqli_query($con,$sql);
		?>
        <script type="text/javascript">
	alert("YOUR TOKEN NUMBER IS :<?php echo $tk;?>");
