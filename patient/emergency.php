...
<header class="header_area">
	...
	</header><section class="team-area section_gap"></section>
	<?php
$con=mysqli_connect("localhost","root","","find_my_doctor");
session_start();
$dt=date("yy-m-d");
$a="SELECT * FROM `token` WHERE pid='".$_SESSION['uid']."'";
$e=mysqli_query($con,$a);
while($f=mysqli_fetch_array($e))
{
	$b="SELECT * FROM `doctor_registration` WHERE id='".$f['did']."'";
	$ex=mysqli_query($con,$b);
	while($r=mysqli_fetch_array($ex))
	{
	
		$c="SELECT * FROM `emergency` WHERE did='".$f['did']."'";
		$x=mysqli_query($con,$c);
		$m=mysqli_fetch_array($x);
		
			if($f['status']=="canceled" && $f['bdate']>=$dt)
			{?>
			
			<center><div style="height:100px; width:800px; background-color:#FF99CC">
   
   <div style="margin-top:30px">
    <?php echo "Dr.".$r['name']." is cancelled consultation on ".$m['date']."due to".$m['message'];?><br />
   <a href="book_token.php?id=<?php echo $f['did'];?>">Click Here</a>
