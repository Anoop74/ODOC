...
session_start();
$s="SELECT distinct id  FROM `consultation` where doctor_id='".$_SESSION['uid']."'";//and id not in(SELECT id FROM `prescription`)
...
$i=1;
  ...
  {
    $a="SELECT distinct patient_id  FROM `consultation`  where id='".$x[0]."'";
	$aa=mysqli_query($con,$a);
  $r=mysqli_fetch_array($aa);
 
  $b="SELECT * FROM `patient_registration` where id='".$r[0]."'";
  $bb=mysqli_query($con,$b);
   ...
   ?>
	...
	$c="SELECT *  FROM `consultation`  where patient_id='".$r[0]."' and id='".$x[0]."'";
	$cc=mysqli_query($con,$c);
   while($n=mysqli_fetch_array($cc))
   {	
	?><tr><td><?php echo $i;?></td>
  ...
  <td><a href="prescription.php?id=<?php echo $x[0];?>" class="genric-btn primary circle">Prescription</a></td></tr>
   <?php
  $i=$i+1;
  }
 }
 ...
 {
  ?><font color="#FF3399"><?php echo "NO DATA FOUND";?></font>
  <?php
 }
  ?>
