...
$np=$_POST['np'];
$cp=$_POST['cp'];
$q="select password from login where rid='".$_SESSION['uid']."' and password='$p' and type='doctor'";
...
if($r=mysqli_fetch_array($e))
	{
	    if($np==$cp)
		 {
		 $up="UPDATE `login` SET `password`= '$np' WHERE rid='".$_SESSION['uid']."' and type='doctor'";
		    mysqli_query($con,$up);
			$upd="UPDATE `doctor_registration` SET `password`= '$np' WHERE id='".$_SESSION['uid']."'";
          ...
          <div class="row">
                ...
                <div class="col-lg-9">
                    ...
                    <form class="row contact_form"  method="post" id="contactForm" novalidate="novalidate" enctype="multipart/form-data">
                        ...
                        <div class="col-md-12">
                            <input type="submit" name="SAVE" value="SAVE" onClick="return validation()" class="btn submit_btn"  style="margin-left:130px">
                        </div>
                        </div>
            </div>                    </form>
                </div>
